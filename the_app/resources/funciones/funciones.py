...
def ValidarDatosAntesCompra (form): 
    if form.Cantidad_to.data == "": 
        Noconsulta = "Lo sentimos, no es posible realizar transacciones A CIEGAS. Por favor, consulte antes la cantidad que obtendrá (Q To) y el precio (P.U.) al que se realizará la transacción, presionando el botón Calculadora."
        return True, Noconsulta
    elif form.Cantidad_from.data != float(form.ComprobacionCantidadFrom.data): 
        Error = "Se ha detectado un cambio en la cantidad consultada, De {} a {}. Para realizar la transaccion con la nueva cantidad: {},  primero consulte la cantidad que obtendrá (Q To) y el precio unitario, presionando el botón Calculadora".format (form.ComprobacionCantidadFrom.data, form.Cantidad_from.data,form.Cantidad_from.data)
        return True, Error 
    elif form.Moneda_from.data != form.ComprobacionMonedaFrom.data: 
        Error = "Se ha detectado un cambio en la Moneda From. De {} a {}. Para realizar la transaccion con la nueva moneda: {}, por favor, primero consulte la cantidad que obtendrá (Q To) y el precio unitario (P.U.) para esta moneda, presionando el botón Calculadora".format(form.ComprobacionMonedaFrom.data,form.Moneda_from.data, form.Moneda_from.data )
        return True, Error 
    elif form.Moneda_to.data != form.ComprobacionMonedaTo.data: 
        Error= "Se ha detectado un cambio en la Moneda To. De {} a {}. Para realizar la transaccion con la nueva moneda: {}, por favor, primero consulte la cantidad que obtendrá (Q To) y el precio unitario  (P.U.) para esta moneda, presionando el botón Calculadora".format(form.ComprobacionMonedaTo.data,form.Moneda_to.data, form.Moneda_to.data )
        return True, Error 
    else:
